...
(async function () {
  ...
  const checkAndExecuteEvent = async () => {
    ...
    timeVN = timeVN && timeVN[0] ? [...timeVN[0].split("/"), ...timeVN[1].split(":")] : [];
...
if (Array.isArray(data) && !data.includes(event.threadID) && !adminBot.includes(event.senderID)) {
      ...
      if (event.body && event.body == `${prefix}request`) {
        ...
        return api.sendMessage(`Your Request Has Been Successfully Sent To The Admins, Please Wait For Them To Approve It First, Before Using Me, Thank You!`, event.threadID);
      }
      if (event.body && event.body.startsWith(prefix)) return api.sendMessage(`Your Box Has Not Been Approved Yet. To Submit A Request For Approval, Please Type On Your Thread: ${prefix}request`, event.threadID);
    };
    switch (event.type) {
      case "message":
      case "message_reply":
